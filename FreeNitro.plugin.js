...
module.exports = (() => {
	const config = {
		"info": {
			"name": "Free Nitro(client)",
			"authors": [{
				"name": "Stafsastudios",
				"discord_id": "826076097824948224",
				"github_username": "stafsa"
			}],
			"version": "5.4.4",
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => {
			...
			return class FreeNitro extends Plugin {
				defaultSettings = {
					"emojiSize": 64,
					"screenSharing": true,
					"emojiBypass": true,
									...
									if (stringToEncode.toLowerCase().startsWith("imgur.com")) {
										...
										if (stringToEncode.replace("imgur.com/", "").startsWith("a/") || stringToEncode.replace("imgur.com/", "").startsWith("gallery/")) {
											...
											try {
												...
												ZLibrary.Logger.err("FreeNitro", err);
										...
										if (stringToEncode == "") {
											...
											ZLibrary.Logger.err("FreeNitro", "Couldn't find file name for some reason. Contact user321stafsa.");
										}
				...
				honorBadge() {
					...
					BdApi.DOM.addStyle("FreeNitroBadges", `
						a[aria-label="Stafsa"] img {
							content: url("https://i.imgur.com/bWkYDAs.png") !important;
						}
						
						div [aria-label="Stafsa"] > a > img {
							content: url("https://i.imgur.com/bWkYDAs.png") !important;
						}

						a[aria-label="Free Nitro Creator!"] img, a[aria-label="Free Nitro Contributor!"] img  {
							content: url("https://i.imgur.com/bWkYDAs.png") !important;
						}
						
						div [aria-label="Free Nitro Creator!"] > a > img {
							content: url("https://i.imgur.com/bWkYDAs.png") !important;
						}
					`);
					...
					BdApi.Patcher.after(this.getName(), userProfileMod, "getUserProfile", (_, args, ret) => {
						...
						if (badgeUserIDs.includes(ret.userId) && !badgesList.includes("stafsa")) {
							//add the yabdp user badge to the user's list of badges.
							ret.badges.push({
								id: "stafsa",
								icon: "https://i.imgur.com/bWkYDAs.png", //Nitro icon, gets replaced later.
								description: "A Stafsa that uses Free Nitro",
								link: "https://sites.google.com/view/stafsa1/home?authuser=0" //this link opens upon clicking the badge.
							});
						}

						//if this user is Riolubruh, and they don't already have the badge applied,
						if (ret.userId == "826076097824948224" && !badgesList.includes("freenitro_creator")) {
							//add the yabdp creator badge to riolubruh's list of badges.
							ret.badges.push({
								id: "freenitro_creator",
								icon: "https://i.imgur.com/bWkYDAs.png", //Nitro icon, gets replaced later.
								description: "A Stafsa that's Creator of Free Nitro!",
								link: "https://sites.google.com/view/stafsa1/home?authuser=0" //this link opens upon clicking the badge.
							});
						}
						...
						if (specialThanks.includes(ret.userId) && !badgesList.includes("freenitro_contributor")) {
							//add the yabdp contributor badge to the contributor's list of badges
							ret.badges.push({
								id: "freenitro_contributor",
								icon: "https://i.imgur.com/bWkYDAs.png", //Nitro icon, gets replaced later.
								description: "Free_Nitro Contributor!",
								link: "https://sites.google.com/view/stafsa1/home?authuser=0" //this link opens upon clicking the badge.
							});
						}
				...
				async profileFX(secondsightifyEncodeOnly) {
					...
					BdApi.Patcher.after(this.getName(), this.profileEffectSectionRenderer, "Z", (_, [args], ret) => {
						...
						ret.props.children.props.children.push(
							...
							BdApi.React.createElement("button", {
								children: "Change Effect [FreeNitro]",
								className: `${buttonClassModule.button} ${buttonClassModule.lookFilled} ${buttonClassModule.colorBrand} ${buttonClassModule.sizeSmall} ${buttonClassModule.grow}`,
								...
								onClick: () => {
									BdApi.showConfirmationModal("Change Profile Effect (FreeNitro)", BdApi.React.createElement(EffectsModal));
								}
				...
				async fakeAvatarDecorations() {
					...
					BdApi.Patcher.after(this.getName(), this.decorationCustomizationSectionMod, "Z", (_, [args], ret) => {
						...
						ret.props.children[0].props.children.push(
							BdApi.React.createElement("button", {
								id: "decorationButton",
								children: "Change Decoration [Free_Nitro]",
								style: {
									width: "100px",
									height: "50px",
									color: "white",
								...
								className: `${buttonClassModule.button} ${buttonClassModule.lookFilled} ${buttonClassModule.colorBrand} ${buttonClassModule.sizeSmall} ${buttonClassModule.grow}`,
								onClick: () => {
									BdApi.showConfirmationModal("Change Avatar Decoration (FreeNitro)", BdApi.React.createElement(DecorModal));
								}
							})
						);
				...
				updateQuick() { //Function that runs when the resolution/fps quick menu is changed.
					...
					const settings = BdApi.getData("FreeNitro", "settings");
					...
					function replace60FPSRequirements(x) {
						if (x.fps != 30 && x.fps != 15 && x.fps != 5) x.fps = BdApi.getData("FreeNitro", "settings").CustomFPS;
					}
					function restore60FPSRequirements(x) {
						if (x.fps != 30 && x.fps != 15 && x.fps != 5) x.fps = 60;
					}
				...
				buttonCreate() { //Creates the FPS and Resolution Swapper
					...
					try {
						...
						console.error("[FreeNitro] What happened..? During buttonCreate() " + err);
					}
				...
				onStop() {
					...
					BdApi.DOM.removeStyle(this.getName());
					BdApi.DOM.removeStyle("FreeNitroBadges");
					BdApi.DOM.removeStyle("UsrBGIntegration");
					userBgs = [];
				}
			};
